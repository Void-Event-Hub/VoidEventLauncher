...
async function populateServerListings(){
    ...
    for(const serv of servers){
        htmlString += `<button class="serverListing" servid="${serv.rawServer.id}" ${
            serv.rawServer.id === giaSel ? 'selected' : ''
        }>
            <div class="serverListingImgContainer">
                <img class="serverListingImg" src="${serv.rawServer.icon}" alt="${serv.rawServer.name}"/>
            </div>
            ...
            ${serv.rawServer.mainServer ? `<div class="serverListingMainWrapper" title="${Lang.queryJS('settings.serverListing.mainServer')}">MAIN MODPACK</div>` : ''}
        </button>`
    }
