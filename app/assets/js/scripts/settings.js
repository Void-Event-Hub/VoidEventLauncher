                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${
                        selectedUUID === acc.uuid
                            ? 'selected>' + Lang.queryJS('settings.authAccountPopulate.selectedAccount')
                            : '>' + Lang.queryJS('settings.authAccountPopulate.selectAccount')
                    }</button>
...
${Object.values(parseModulesForUI(mdl.subModules, true, servConf[mdl.getVersionlessMavenIdentifier()])).join('')}
                ...
                optMods += `<div id="${mdl.getVersionlessMavenIdentifier()}" class="settingsBaseMod settings${submodules ? 'Sub' : ''}Mod" ${
                    val ? 'enabled' : ''
                }>
                    <div class="settingsModContent">
                        <div class="settingsModMainWrapper">
                        ...
                        <div class="settingsModMainWrapper">
                            ...
                            <div class="settingsModDetails">
                                ...
                                <div class="settingsDropinRemoveWrapper">
                                    <button class="settingsDropinRemoveButton" remmod="${dropin.fullName}">${Lang.queryJS(
        ...
        if (dropinUI != null) {
            ...
            if (DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled) {
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if (!isOverlayVisible()) {
                        setOverlayContent(Lang.queryJS('settings.dropinMods.failedToggleTitle'), err.message, Lang.queryJS('settings.dropinMods.okButton'))
                        setOverlayHandler(null)
                        toggleOverlay(true)
                    }
                })
...
function createRAMInputFields() {
    ...
    maxRAMInput.max = settingsMaxRAMRange.getAttribute('max')
    ...
    minRAMInput.max = settingsMinRAMRange.getAttribute('max')
    ...
    maxRAMInput.addEventListener('change', e => {
        ...
        const cappedValue = Math.max(sliderMeta.min, Math.min(value, sliderMeta.max))
        ...
        // Update Allocated value live
        settingsMemoryAvail.innerHTML = e.target.value + 'G'
    })
...
function bindRangeSlider() {
    Array.from(document.getElementsByClassName('rangeSlider')).forEach(slider => {
        // Remove existing notches
        Array.from(slider.getElementsByClassName('rangeSliderNotch')).forEach(n => n.remove())

        // Get slider meta
        const sliderMeta = calculateRangeSliderMeta(slider)
        const min = sliderMeta.min
        const max = sliderMeta.max
        const step = sliderMeta.step
        const ticks = sliderMeta.ticks

        // Add notches for each 0.1G step
        for (let i = 0; i <= ticks; i++) {
            const notch = document.createElement('div')
            notch.className = 'rangeSliderNotch'
            const percent = (i / ticks) * 100
            notch.style.left = percent + '%'
            slider.appendChild(notch)
        }

        // Reference the track (thumb) and bar
        const track = slider.getElementsByClassName('rangeSliderTrack')[0]
        ...
        function setSliderUI(val) {
            ...
            // Update the input field if present
            if (slider === settingsMaxRAMRange) {
                const maxRAMInput = document.getElementById('settingsMaxRAMInput')
                if (maxRAMInput) maxRAMInput.value = val.toFixed(1)
                // Update Allocated value live
                settingsMemoryAvail.innerHTML = val.toFixed(1) + 'G'
            } else if (slider === settingsMinRAMRange) {
                const minRAMInput = document.getElementById('settingsMinRAMInput')
                if (minRAMInput) minRAMInput.value = val.toFixed(1)
            }
        }

        // Helper to get value from mouse event
        function getValueFromEvent(e) {
            const rect = slider.getBoundingClientRect()
            const x = e.touches ? e.touches[0].clientX : e.clientX
            let clickPos = x - rect.left
            clickPos = Math.max(0, Math.min(clickPos, rect.width))
        ...
        slider.onclick = e => {
            if (e.target === track) return // Let drag handle this
            const newValue = getValueFromEvent(e)
            setSliderUI(newValue)
            dispatchSliderChange(slider, newValue)
        }

        // Drag logic for the track (thumb)
        let dragging = false
        function onMove(e) {
            if (!dragging) return
            const newValue = getValueFromEvent(e)
            ...
            // Update allocated RAM display if this is the max RAM slider
            if (slider === settingsMaxRAMRange) {
                settingsMemoryAvail.innerHTML = newValue.toFixed(1) + 'G'
                const maxRAMInput = document.getElementById('settingsMaxRAMInput')
                if (maxRAMInput) maxRAMInput.value = newValue.toFixed(1)
            }
            dispatchSliderChange(slider, newValue)
        }
        function onUp() {
...
function dispatchSliderChange(slider, value) {
    // Fire a change event for the slider div
...
settingsMaxRAMRange.onchange = e => {
    ...
    // Update Allocated value live
    settingsMemoryAvail.innerHTML = sMaxV.toFixed(1) + 'G'
}
...
function populateMemoryStatus() {
    ...
    // Show the max RAM slider value as the allocated value
    const allocatedRAM = Number(settingsMaxRAMRange.getAttribute('value')).toFixed(1)

    settingsMemoryTotal.innerHTML = totalRAM + 'G'
    settingsMemoryAvail.innerHTML = allocatedRAM + 'G'
}
...
function bindMinMaxRam(server) {
    ...
    // Get total RAM and subtract 1GB for system overhead
    const totalRAM = Number((os.totalmem() - 1073741824) / 1073741824)
    const cappedMaxMemory = Math.min(SETTINGS_MAX_MEMORY, totalRAM)
