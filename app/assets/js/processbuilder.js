...
class ProcessBuilder {
    ...
    build() {
        ...
        if (process.env['_JAVA_OPTIONS'] && os.platform() === 'win32') {
            ...
            logger.warn(
                'Detected _JAVA_OPTIONS environment variable. This can cause issues with the launcher, so it has been overridden.'
            )
        }
                ...
                if (lib.natives != null) {
                    ...
                    const artifact =
                        lib.downloads.classifiers[
                            lib.natives[getMojangOS()].replace('${arch}', process.arch.replace('x', ''))
                        ]
